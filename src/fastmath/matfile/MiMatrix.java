...
public class MiMatrix extends MiElement {
	...
	public MiMatrix(String values, String name) {
		...
		this.arrayFlags.setElementAt(1, 0L);
		this.dimensionsArray = new MiInt32(1, values.length());
		this.arrayName = new MiInt8(name);
	}
	...
	public MiMatrix(int[] values, String name) {
		...
		this.arrayFlags.setElementAt(0, this.value.getArrayType().getType());
		this.arrayFlags.setElementAt(1, 0L);
		this.dimensionsArray = new MiInt32(values.length, 1);
		this.arrayName = new MiInt8(name);
	}
	...
	protected void readValue(Iterator<MiElement> iter) {
		...
		switch (arrayClass) {
		case 12: {
			this.value = new MxInt32(iter, this.isComplex());
			break;
		}
		case 4: {
			this.value = new MxChar(iter, this.isComplex());
			break;
		}
		case 6: {
			this.value = new MxDouble(iter, this.isComplex(), this.dimensionsArray);
			break;
		}
		case 2: {
		...
		default: {
			throw new UnsupportedOperationException(
					"Unsupported type of arrayClass " + (Object) ((Object) MxClass.Type.valueOf(arrayClass)));
		}
		}
	}
	...
	public String toString() {
		return "MiMatrix\"" + this.getName() + "\"[" + this.dimensionsArray + "]\"" + this.arrayName + "\"{"
				+ this.value + "}";
	}
