...
import static fastmath.Constants.DBL_MAX_EXP;
import static fastmath.Constants.DBL_MIN;
import static fastmath.Constants.DBL_MIN_EXP;
import static fastmath.Constants.M_1_SQRT_2PI;
import static fastmath.Constants.M_LN2;
import static fastmath.Constants.M_LN_SQRT_2PI;
import static fastmath.Constants.M_LN_SQRT_PId2;
import static fastmath.Constants.M_LOG_PI;
import static fastmath.Constants.M_SQRT_PI;
...
public class MathFunctions
{
   ...
   *
   * These routines are translations into C of Fortran routines by W. Fullerton of
   * Los Alamos Scientific Laboratory.
   *
   * Based on the Fortran routine dcsevl by W. Fullerton. Adapted from R. Broucke,
   * Algorithm 446, CACM., 16, 254 (1973).
   */
  ...
  public static final double lgammafn_sign(double x, int[] sgn)
  {
    ...
    { /*
       ...
       */
      // MATHLIB_WARNING(" ** should NEVER happen! *** [lgamma.c: Neg.int,
      // y=%g]\n",y);
      return Double.NaN;
    }
    ...
    {
      /*
       * The answer is less than half precision because the argument is too near a
       * negative integer.
       */
      // ML_ERROR(ME_PRECISION, "lgamma");
      System.err.println("lgamma precision error!");
      if (Debug.warningAsError)
        throw new RuntimeException("lgamma precision error!");
    }
    return ans;
  }
  ...
  {
    final double S0 = 0.083333333333333333333, S1 = 0.00277777777777777777778, S2 = 0.00079365079365079365079365,
                  S3 = 0.000595238095238095238095238, S4 = 0.0008417508417508417508417508;
  ...
  public static final double gammafn(double x)
  {
    ...
    double y, value;
    /*
     * int ngam = 0; double xmin = 0., xmax = 0., xsml = 0., dxrel = 0.; if (ngam ==
     * 0) { ngam = chebyshev_init(gamcs, 42, DBL_EPSILON/20);
     * xmin=-170.5674972726612; xmax=171.61447887182298; xsml =
     * exp(max(log(DBL_MIN), -log(Double.MAX_VALUE))+0.01); dxrel =
     * sqrt(DBL_EPSILON); } /
     */
    final int ngam = 22;
    final double xmin = -170.5674972726612, xmax = 171.61447887182298, xsml = 2.2474362225598545e-308,
  ...
  /*
   * Evaluates the "deviance part" bd0(x,M) := M * D0(x/M) = M*[ x/M * log(x/M) +
   * 1 - (x/M) ] = = x * log(x/M) + M - x where M = E[X] = n*p (or = lambda), for
   * x, M > 0
   *
   * in a manner that should be stable (with small relative error) for all x and
   * M=np. In particular for x/np close to 1, direct evaluation fails, and
   * evaluation is based on the Taylor series of log((1+v)/(1-v)) with v =
   * (x-M)/(x+M) = (x-np)/(x+np).
   */
  ...
  protected static final double bigx = 4294967296.0,
                scalefactor = bigx * bigx * bigx * bigx * bigx * bigx * bigx * bigx;
  // ;

  /*
   * Continued fraction for calculation of 1/i + x/(i+d) + x^2/(i+2*d) +
   * x^3/(i+3*d) + ... = sum_{k=0}^Inf x^k/(i+k*d)
   *
  ...
  public static final double log1pmx(double x)
  {
    ...
    { /*
       * -.791 <= x <= 1 -- expand in [x/(2+x)]^2 =: y : log(1+x) - x = x/(2+x) * [ 2
       * * y * S(y) - x], with --------------------------------------------- S(y) =
       * 1/3 + y/5 + y^2/7 + ... = \sum_{k=0}^\infty y^k / (2k + 3)
       */
  ...
  /**
   * log1px takes a double and returns a double. It is a Taylor series expansion
   * of log(1+x). x is presumed to be < 1. As I have called it, x < .1, and so I
   * know the algorithm will terminate quickly. The closer x is to 1, the slower
   * this will be. (From AS 885)
   */
  public static final double log1px(double x)
  {
    ...
    xn = x;
    /*
     * Comparing ans!=oans is done here to insure that this calculation continues
     * until the accuracy of the machine is reached. At some point, the value is not
  ...
  public static final double lgamma1p(double a)
  {
    ...
    /*
     * Abramowitz & Stegun 6.1.33 : for |x| < 2, <==> log(gamma(1+x)) = -(log(1+x) -
     * x) - gamma*x + x^2 * \sum_{n=0}^\infty c_n (-x)^n where c_n := (Zeta(n+2) -
     * 1)/(n+2) = coeffs[n]
     *
     * Here, another convergence acceleration trick is used to compute lgam(x) :=
     * sum_{n=0..Inf} c_n (-x)^n
     */
   ...
   *
   * without causing overflows and without throwing away large handfuls of
   * accuracy.
   */
   ...
   * Compute the log of a sum from logs of terms, i.e.,
   *
   * log (sum_i exp (logx[i]) ) = log (e^M * sum_i e^(logx[i] - M) ) = M + log(
   * sum_i e^(logx[i] - M)
   *
  ...
  public static final double logspace_sum(double[] logx)
  {
    int n = logx.length;
    if (n == 0)
      return Double.NEGATIVE_INFINITY; // = log( sum(<empty>) )
    if (n == 1)
      return logx[0];
    if (n == 2)
  ...
  /*
   * Based on C translation of ACM TOMS 708 Please do not change this, e.g. to use
   * R's versions of the ancillary routines, without investigating the error
   * analysis as we do need very high relative accuracy. This version has about 14
   * digits accuracy.
   */
  /**
   * <pre>
  ...
  {
    boolean do_swap;
    int n = 0, kase = 0, ierr1[] =
    { 0 };
    double z, a0, b0, x0, y0, lambda = 0, w, w1;

    /*
     * eps is a machine dependent constant: the smallest floating point number for
     * which 1.0 + eps > 1.0
     */
    ...
    if (min(a, b) <= 1.)
    {
      ...
      if (kase == 0 || kase == 131)
      {
        ...
        { // w1=0 or very close:
          ...
          return new double[]
          { w, w1, (ierr1[0] > 0) ? 10 + ierr1[0] : 0 };
        }
        // if(w1 < 0) MATHLIB_WARNING4("bratio(a=%g, b=%g, x=%g): bgrat() -> w1 = %g",
        // a,b,x, w1);
        // goto L_end_from_w1;
        if (log_p)
        {
    ...
    else
    {
      ...
      lambda = MathFunctions.isFinite(a + b) ? ((a > b) ? (a + b) * y - b : a - (a + b) * x) : a * y - b * x;
  ...
  public static final double fpser(double a, double b, double x, double eps, boolean log_p)
  {
    /*
     * ----------------------------------------------------------------------- *
     * EVALUATION OF I (A,B) X FOR B < MIN(EPS, EPS*A) AND X <= 0.5
     * -----------------------------------------------------------------------
     */
  ...
  public static final double apser(double a, double b, double x, double eps)
  {
    /*
     * -----------------------------------------------------------------------
     * apser() yields the incomplete beta ratio I_{1-x}(b,a) for a <=
     * min(eps,eps*b), b*x <= 1, and x <= 0.5, i.e., a is very small. Use only if
     * above inequalities are satisfied.
     * -----------------------------------------------------------------------
     */
  ...
  public static final double bpser(double a, double b, double x, double eps, boolean log_p)
  {
    /*
     * ----------------------------------------------------------------------- Power
     * SERies expansion for evaluating I_x(a,b) when b <= 1 or b*x <= 0.7. eps is
     * the tolerance used.
     * -----------------------------------------------------------------------
     */
    ...
    { // the series did not converge (in time)
      ...
      if ((log_p && !(a * sum > -1. && abs(log1p(a * sum)) < eps * abs(ans))) || (!log_p && abs(a * sum + 1.) != 1.))
      {
        String errstr = String.format(" bpser(a=%g, b=%g, x=%g,...) did not converge (n=1e7, |w|/tol=%g > 1; A=%g)",
                                      a,
                                      b,
                                      x,
                                      abs(w) / tol,
    ...
    if (log_p)
    {
      ...
      else
      {
        if (ans > Double.NEGATIVE_INFINITY && Debug.warningAsError)
          throw new PrecisionException(String.format("pbeta(*, log.p=TRUE) -> bpser(a=%g, b=%g, x=%g,...) underflow to -Inf",
                                                     a,
                                                     b,
                                                     x),
                                       ans);
  ...
  public static final double bfrac(double a, double b, double x, double y, double lambda, double eps, boolean log_p)
  {
    /*
     * -----------------------------------------------------------------------
     * Continued fraction expansion for I_x(a,b) when a, b > 1. It is assumed that
     * lambda = (a + b)*y - b.
     * -----------------------------------------------------------------------
     */
    ...
    { /* already underflowed to 0 */
      if (Debug.warningAsError)
        throw new PrecisionException(" --> brcomp(a,b,x,y) underflowed to 0.\n",
                                     0);
      return 0.;
    }
  ...
  public static final double brcmp1(int mu, double a, double b, double x, double y, boolean give_log)
  {
    ...
    else
    {
      ...
      return give_log ? log(M_1_SQRT_2PI) + (log(b) + lx0) / 2. + z - bcorr(a, b)
                    : M_1_SQRT_2PI * sqrt(b * x0) * z * exp(-bcorr(a, b));
    }
  } /* brcmp1 */

  public static final double
         bgrat(double a, double b, double x, double y, double w, double eps, int[] ierr, boolean log_w)
  {
    /*
     * -----------------------------------------------------------------------
     * Asymptotic Expansion for I_x(A,B) when a is larger than b. The result of the
     * expansion is added to w. It is assumed a >= 15 and b <= 1. eps is the
     * tolerance used. ierr is a variable that reports the status of the results.
     * -----------------------------------------------------------------------
     */
    final int n_terms_bgrat = 30;
    double c[] = new double[n_terms_bgrat], d[] = new double[n_terms_bgrat];
    double bm1 = b - 0.5 - 0.5, nu = a + bm1 * 0.5, /*
                                                     * nu = a + (b-1)/2 =: T, in (9.1) of Didonato & Morris(1992),
                                                     * p.362
                                                     */
                  lnx = (y > 0.375) ? log(x) : alnrel(-y), z = -nu * lnx; // z =: u in (9.1) of D.&M.(1992)
    ...
    double
    /*
     * r1 = b * (gam1(b) + 1.0) * exp(b * log(z)),// = b/gamma(b+1) z^b = z^b /
     * gamma(b) set r := exp(-z) * z^b / gamma(b) ; gam1(b) = 1/gamma(b+1) - 1 , b
     * in [-1/2, 3/2]
     */
    // exp(a*lnx) underflows for large (a * lnx); e.g. large a ==> using log_r :=
    // log(r):
    ...
    double l = // := *w/u .. but with care: such that it also works when u underflows to 0:
                  log_w ? ((w == Double.NEGATIVE_INFINITY) ? 0. : exp(w - log_u))
    ...
    for (int n = 1; n <= n_terms_bgrat; ++n)
    {
      ...
      { // never? ; please notify R-core if seen:
        ierr[0] = 4;
        // MATHLIB_WARNING4("bgrat(a=%g, b=%g, x=%g,..): did *not* converge;
        // rel.err=%g",
        // a,b,x, abs(dj) /(sum + l));
      }
    }
  ...
  /*
   * -----------------------------------------------------------------------
   * Scaled complement of incomplete gamma ratio function grat_r(a,x,r) := Q(a,x)
   * / r where Q(a,x) = pgamma(x,a, lower.tail=FALSE) and r = e^(-x)* x^a /
   * Gamma(a) == exp(log_r)
   *
  ...
  public static final double basym(double a, double b, double lambda, double eps, boolean log_p)
  {
    ...
    // #define num_IT 20
    final int num_IT = 20, nip1 = num_IT + 1;
    ...
    double a0[] = new double[nip1], b0[] = new double[nip1], c[] = new double[nip1], d[] = new double[nip1];
  ...
  public static final double alnrel(double a)
  {
    if (abs(a) > 0.375)
      return log(1. + a);
    final double p1 = -1.29418923021993, p2 = .405303492862024, p3 = -.0178874546012214, q1 = -1.62752256355323,
  ...
  public static final double erf__(double x)
  {
    ...
    final double a[] =
    { 7.7105849500132e-5, -.00133733772997339, .0323076579225834, .0479137145607681, .128379167095513 };
    final double b[] =
    { .00301048631703895, .0538971687740286, .375795757275549 };
    final double p[] =
    { -1.36864857382717e-7, .564195517478974, 7.21175825088309, 43.1622272220567, 152.98928504694, 339.320816734344,
        451.918953711873, 300.459261020162 };
    final double q[] =
    { 1., 12.7827273196294, 77.0001529352295, 277.585444743988, 638.980264465631, 931.35409485061, 790.950925327898,
        300.459260956983 };
    final double r[] =
   ...
   *
   * ERFC1(IND,X) = ERFC(X) IF IND = 0 ERFC1(IND,X) = EXP(X*X)*ERFC(X) OTHERWISE
   * -----------------------------------------------------------------------
   */
  ...
  {
    final double p[] =
    { .577215664901533, -.409078193005776, -.230975380857675, .0597275330452234, .0076696818164949,
        -.00514889771323592, 5.89597428611429e-4 };
    final double q[] =
    { 1., .427569613095214, .158451672430138, .0261132021441447, .00423244297896961 };
    final double r[] =
    { -.422784335098468, -.771330383816272, -.244757765222226, .118378989872749, 9.30357293360349e-4,
        -.0118290993445146, .00223047661158249, 2.66505979058923e-4, -1.32674909766242e-4 };
  ...
  public static final double psi(double x)
  {
    ...
    final double p1[] =
    { .0089538502298197, 4.77762828042627, 142.441585084029, 1186.45200713425, 3633.51846806499, 4138.10161269013,
        1305.60269827897 };
    final double q1[] =
    { 44.8452573429826, 520.752771467162, 2210.0079924783, 3641.27349079381, 1908.310765963, 6.91091682714533e-6 };
    /* --------------------------------------------------------------------- */
    ...
    double p2[] =
    { -2.12940445131011, -7.01677227766759, -4.48616543918019, -.648157123766197 };
    double q2[] =
    { 32.2703493791143, 89.2920700481861, 54.6117738103215, 7.77788548522962 };
    /* --------------------------------------------------------------------- */
     ...
     * ARGUMENTS AND AS THE POSITIVE ARGUMENT BEYOND WHICH PSI MAY BE REPRESENTED AS
     * LOG(X). Originally: xmax1 = amin1(ipmpar(3), 1./spmpar(1))
     */
  ...
  /**
   * -----------------------------------------------------------------------
   * EVALUATION OF DEL(A0) + DEL(B0) - DEL(A0 + B0) WHERE LN(GAMMA(A)) = (A -
   * 0.5)*LN(A) - A + 0.5*LN(2*PI) + DEL(A). IT IS ASSUMED THAT A0 >= 8 AND B0 >=
   * 8. -----------------------------------------------------------------------
   */
  ...
  /**
   * -----------------------------------------------------------------------
   * COMPUTATION OF LN(GAMMA(B)/GAMMA(A+B)) WHEN B >= 8. IN THIS ALGORITHM, DEL(X)
   * IS THE FUNCTION DEFINED BY LN(GAMMA(X)) = (X - 0.5)*LN(X) - X + 0.5*LN(2*PI)
   * + DEL(X).
   * -----------------------------------------------------------------------
   */
  ...
  public static final double beta(double a, double b)
  {
    ...
    xmax = 171.61447887182298, lnsml = -708.39641853226412;
  ...
  public static final double gamma_cody(double x)
  {
     ...
     * positive power of beta that overflows XBIG - the largest argument for which
     * GAMMA(X) is representable in the machine, i.e., the solution to the equation
     * GAMMA(XBIG) = beta**maxexp XINF - the largest machine representable
     * floating-point number; approximately beta**maxexp EPS - the smallest positive
     * floating-point number such that 1.0+EPS > 1.0 XMININ - the smallest positive
     * floating-point number such that 1/XMININ is machine representable
     * 
     * Approximate values for some important machines are:
     * 
     * beta maxexp XBIG
     * 
     * CRAY-1 (S.P.) 2 8191 966.961 Cyber 180/855 under NOS (S.P.) 2 1070 177.803
     * IEEE (IBM/XT, SUN, etc.) (S.P.) 2 128 35.040 IEEE (IBM/XT, SUN, etc.) (D.P.)
     * 2 1024 171.624 IBM 3033 (D.P.) 16 63 57.574 VAX D-Format (D.P.) 2 127 34.844
     * VAX G-Format (D.P.) 2 1023 171.489
     * 
     * XINF EPS XMININ
     * 
     * CRAY-1 (S.P.) 5.45E+2465 7.11E-15 1.84E-2466 Cyber 180/855 under NOS (S.P.)
     * 1.26E+322 3.55E-15 3.14E-294 IEEE (IBM/XT, SUN, etc.) (S.P.) 3.40E+38 1.19E-7
     * 1.18E-38 IEEE (IBM/XT, SUN, etc.) (D.P.) 1.79D+308 2.22D-16 2.23D-308 IBM
     * 3033 (D.P.) 7.23D+75 2.22D-16 1.39D-76 VAX D-Format (D.P.) 1.70D+38 1.39D-17
     * 5.88D-39 VAX G-Format (D.P.) 8.98D+307 1.11D-16 1.12D-308
     *******************************************************************
     * 
     * 
     * ----------------------------------------------------------------------
    ...
    { -1.71618513886549492533811, 24.7656508055759199108314, -379.804256470945635097577, 629.331155312818442661052,
        866.966202790413211295064, -31451.2729688483675254357, -36144.4134186911729807069,
        66456.1438202405440627855 };
    final double q[] =
    { -30.8402300119738975254353, 315.350626979604161529144, -1015.15636749021914166146, -3107.77167157231109440444,
        22538.1184209801510330112, 4755.84627752788110767815, -134659.959864969306392456,
    ...
    else if (y < 12.)
    {
      ...
      else
      {
        /*
         * ----------------------------------------------------------- 1 <= argument <
         * 12, reduce argument if necessary
         * -----------------------------------------------------------
         */
        n = (long) y - 1;
        y -= (double) n;
      ...
      /*
       * --------------------------------------------------------- Evaluate
       * approximation for 1. < argument < 2.
       * ---------------------------------------------------------
       */
      xnum = 0.;
      xden = 1.;
  ...
  public static final double pd_upper_series(double x, double y, boolean log_p)
  {
     ...
     * (y*(y+1)*...*(y+n)) = x/y * (1 + \sum_{n=1}^oo x^n / ((y+1)*...*(y+n))) ~ x/y
     * + o(x/y) {which happens when alph -> Inf}
     */
    return log_p ? log(sum) : sum;
  }

  /*
   * Continued fraction for calculation of scaled upper-tail F_{gamma} ~= (y / d)
   * * [1 + (1-y)/d + O( ((1-y)/d)^2 ) ]
   */
  ...
  public static final double pd_lower_series(double lambda, double y)
  {
    ...
    /*
     * sum = \sum_{n=0}^ oo y*(y-1)*...*(y - n) / lambda^(n+1) = y/lambda * (1 +
     * \sum_{n=1}^Inf (y-1)*...*(y-n) / lambda^n) ~ y/lambda + o(y/lambda)
     */

    if (y != floor(y))
    {
      /*
       * The series does not converge as the terms start getting bigger (besides
       * flipping sign) for y < -lambda.
       */
      double f;
      /*
  ...
  {
    /*
     * series for log1p on the interval -.375 to .375 with weighted error 6.35e-32
     * log weighted error 31.20 significant figures required 30.93 decimal places
     * required 32.01
     */
    final double alnrcs[] =
    ...
    if (x < xmin)
    {
      ...
      if (Debug.warningAsError)
        throw new RuntimeException("Precision loss warning at log1p");
  ...
  /*
   * Compute log(1 + exp(x)) without overflow (and fast for x > 18) For the two
   * cutoffs, consider curve(log1p(exp(x)) - x, 33.1, 33.5, n=2^10)
   * curve(x+exp(-x) - log1p(exp(x)), 15, 25, n=2^11)
   */
  public static final double log1pexp(double x)
  { // LogLogistic routine needs this
    if (x <= 18.)
      return log1p(exp(x));
    if (x > 33.3)
      return x;
